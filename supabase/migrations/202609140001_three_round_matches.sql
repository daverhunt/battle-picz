...
begin
  ...
  select coalesce(m.game_config, '{}'::jsonb) - 'rounds'
  into config
...
begin
  ...
  if p_round_no not between 1 and 3 then
  ...
  if current_config is null then
    raise exception 'Both players must join before the round can start';
  end if;
...
begin
  ...
  if match_status is distinct from 'active' then
